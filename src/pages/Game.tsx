    ...
    <div className="w-full flex flex-col space-y-6">
      <div className="flex justify-between items-center bg-slate-800/50 p-6 rounded-3xl border border-slate-700/50 backdrop-blur-sm">
        <div className="flex flex-col space-y-1">
          ...
          <span onClick={copyRoomCode} className="text-3xl font-black font-mono text-purple-400 select-all cursor-pointer tracking-wider hover:text-purple-300 transition-colors">
            {hostId}
          </span>
        </div>
      ...
      <div className="mt-8 border-t border-slate-800 pt-8">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {players.map(p => (
            <div key={p.id} className={`p-4 rounded-2xl border-2 transition-all ${p.id === hostId ? 'border-purple-500/50 bg-purple-500/5 shadow-lg shadow-purple-500/5' : 'border-slate-800 bg-slate-800/50'} flex justify-between items-center group`}>
              <div className="flex flex-col">
                <span className={`font-bold truncate ${p.id === playerId ? 'text-white' : 'text-slate-400'}`}>
                  {p.name} {p.id === playerId && '★'}
                </span>
                {p.id === hostId && <span className="text-[8px] text-purple-500 font-black uppercase tracking-tighter">Host</span>}
              </div>
              <span className="bg-slate-900 px-3 py-1 rounded-full text-xs font-black text-purple-400 border border-slate-700">{p.score}</span>
            </div>
          ))}
        </div>
      </div>
